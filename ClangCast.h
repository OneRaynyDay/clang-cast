...
CXXCast castHelper(const Expr *Expression, CXXCast Cast,
  ...
  Cast =
      std::max(Cast, cppcast::getCastType(CastExpression, CanonicalSubExprType,
                                          CanonicalCastType));
  return castHelper(CastExpression->getSubExpr(), Cast, CanonicalSubExprType,
                    CanonicalCastType);
}
...
bool recurseDowncastCheck(const QualType &SubExpr, const QualType &CastType,
                          const ASTContext *Context, const bool Pedantic) {
...
bool requireConstCast(const CStyleCastExpr *CastExpression,
  ...
  QualType CanonicalSubExprType =
      CastExpression->getSubExpr()->getType().getCanonicalType();
  ...
  if (details::isFunctionPtr(CanonicalCastType) ||
      details::isFunctionPtr(CanonicalSubExprType)) {
    return false;
  }
  ...
  return details::recurseDowncastCheck(CanonicalSubExprType, CanonicalCastType,
                                       Context, Pedantic);
}
...
CXXCast getCastKindFromCStyleCast(const CStyleCastExpr *CastExpression) {
  ...
  return details::castHelper(CastExpression,
                             ...
                             CXXCast::CC_NoOpCast, CanonicalSubExpressionType,
                             CanonicalCastType);
}
...
/// (int***) x;
///
/// will yield (int* const* const* const) because const_cast will only modify
/// the qualifiers where they are both locally similar (they stop becoming
/// similar at array vs. pointer)
///
...
/// cast \param To, the SubExpression, which has specific qualifiers on it.
/// \param Context, ASTContext for helper
/// \return QualType mirroring From but with qualifiers on To.
QualType changeQualifiers(QualType From, const QualType &To,
                          const ASTContext *Context, const bool Pedantic) {
...
CXXCast getCastType(const CastExpr *CastExpression,
  ...
  case CastKind::CK_DerivedToBase: {
    ...
    const CXXRecordDecl *Derived =
        (*CanonicalSubExprType).getPointeeCXXRecordDecl();
  ...
  case CastKind::CK_BaseToDerived: {
    const CXXRecordDecl *Base =
        (*CanonicalSubExprType).getPointeeCXXRecordDecl();
    const CXXRecordDecl *Derived =
        (*CanonicalCastType).getPointeeCXXRecordDecl();
    return details::getBaseDerivedCast(Base, Derived);
  }
